...
def describe_game(name):
    """
        ...
        If is is new, get the user's name.
        If it is not a new game, thank the player for
        playing again and continue with the game
    """
    # meaning, if we do not already have this user's name,
    # then they are a new player and we need to their name
    if name !="":
...
def win(nice,mean,name):
    # subsitutue the {} wildcars with our variable values
...
def reset(nice,mean,name):
    nice = 0
    mean = 0
    #notice, i do not reset the name variable as that same user had elected to play again
